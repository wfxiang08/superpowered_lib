...
/**
 ...
 @param pitchShift 
    Pitch shift notes, from -12 (one octave down) to 12 (one octave up). 0 means no pitch shift. Read only.
 @param pitchShiftCents 
    Pitch shift cents, from -2400 (two octaves down) to 2400 (two octaves up). 0 means no pitch shift. Read only.
...
public:
    ...
    /**
     ...
     @param newRate Limited to >= 0.01f and <= 4.0f. 
            Values above 2.0f or below 0.5f are not recommended on mobile devices with low latency audio.
            TimeStretching这个确实不是我们需要的，默认设置为为1.0f即可
    ...
    /**
     为了性能这个函数就不要使用了
     @brief Set rate and pitch shift with greater precision. 
            Calling this method requires magnitudes more CPU than setRateAndPitchShift.
     @param newRate Limited to >= 0.01f and <= 4.0f. 
        Values above 2.0f or below 0.5f are not recommended on mobile devices with low latency audio.
    ...
    */
    ...
    /**
     @brief This class handles one stereo audio channel pair by default. 
